...
#' Function that calls to estimate19 for a set of parameters
...
sexp19 <- function(x, param, rp, iteration) {
  ...
  h <- parallel::parSapply(cl, n1:n2, function(y) {
    elapsedTime <- microbenchmark::microbenchmark(z <- estimate19(x, y, alpha1, alpha2), times=1)
    # plot(z[[1]], type = "l")
    l <- kl(z$estimate, rp)

    # saves the results in a file
    saveResult(method=19, iteration=iteration, args=c(n=y, alpha1=alpha1, alpha2=alpha2),
