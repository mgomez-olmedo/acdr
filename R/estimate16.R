...
#' Function that calls to estimate16 for a set of parameters
...
sexp16 <- function(x, param, rp, iteration) {
  alpha1 <- as.numeric(param[2])
  alpha2 <- as.numeric(param[3])

  elapsedTime <- microbenchmark::microbenchmark(h <- estimate16(x, alpha1, alpha2), times=1)

  # plot(h[[1]], type = "l")
  l <- kl(h$estimate, rp)

  # saves the results in a file
  saveResult(method=16, iteration=iteration, args=c(alpha1=alpha1, alpha2=alpha2),
