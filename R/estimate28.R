...
#' Function that calls to estimate28 for a set of parameters
...
sexp28 <- function(x, param, rp, iteration) {
  ...
  h <- parallel::parSapply(cl, n1:n2, function(y) {
    elapsedTime <- microbenchmark::microbenchmark(z <- estimate28(x, y, alpha), times=1)
    # plot(z[[1]], type = "l")
    l <- kl(z$estimate, rp)

    # saves the results in a file
    saveResult(method=28, iteration=iteration, args=c(n=y, alpha=alpha),
