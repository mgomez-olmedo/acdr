...
estimate29 <- function(x, n, alpha) {
  y <- vector(0, length(x))
  s   <- vector(0, length(x))
  ro <- rep(1, length(x))
  fg <- vector(0, length(x))
  r <- vector(0, length(x))
...
#' Function that calls to estimate29 for a set of parameters
...
sexp29 <- function(x, param, rp, iteration) {
  ...
  h <- parallel::parSapply(cl, n1:n2, function(y) {
    elapsedTime <- microbenchmark::microbenchmark(z <- estimate29(x, y, alpha), times=1)
    # plot(z[[1]], type = "l")
    l <- kl(z$estimate, rp)

    # saves the results in a file
    saveResult(method=29, iteration=iteration, args=c(n=y, alpha=alpha),
