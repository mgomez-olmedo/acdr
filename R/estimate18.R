...
#' Function that calls to estimate18 for a set of parameters
...
sexp18 <- function(x, param, rp, iteration) {
  alpha1 <- as.numeric(param[2])
  alpha2 <- as.numeric(param[3])

  elapsedTime <- microbenchmark::microbenchmark(h <- estimate18(x, alpha1, alpha2), times=1)
  # plot(h[[1]], type = "l")
  l <- kl(h$estimate, rp)

  # saves the results in a file
  saveResult(method=18, iteration=iteration, args=c(alpha1=alpha1, alpha2=alpha2),
