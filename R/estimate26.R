...
#' Function that calls to estimate26 for a set of parameters
...
sexp26 <- function(x, param, rp, iteration) {
  ...
  h <- parallel::parSapply(cl, n1:n2, function(y) {
    elapsedTime <- microbenchmark::microbenchmark(z <- estimate26(x, y, alpha), times=1)
    # plot(z[[1]], type = "l")
    l <- kl(z$estimate, rp)

    # saves the results in a file
    saveResult(method=26, iteration=iteration, args=c(n=y, alpha=alpha),
