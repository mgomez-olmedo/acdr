#' Estimation method named SWMTBIn
...
#' Function that calls to estimate24 for a set of parameters
...
sexp24 <- function(x, param, rp, iteration) {
  ...
  h <- parallel::parSapply(cl, n1:n2, function(y) {
    elapsedTime <- microbenchmark::microbenchmark(z <- estimate24(x, y, alpha), times=1)
    # plot(z[[1]], type = "l")
    l <- kl(z$estimate, rp)

    # saves the results in a file
    saveResult(method=24, iteration=iteration, args=c(n=y, alpha=alpha),
